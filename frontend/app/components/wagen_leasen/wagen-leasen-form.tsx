...
export default function WagenLeasenForm({ wagenName, onClose, onLeasenSuccess }: WagenLeasenFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // TODO: Endpunkte hier aufrufen
  };

  return (
    <div className="relative text-white animate-scaleIn w-full max-w-sm rounded-2xl bg-[#121C27] p-12 shadow-xl">

      <h2 className="mb-8 text-center text-3xl font-bold text-gray-200">{wagenName} leasen</h2>

      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="flex items-center justify-between">
          <label className="text-[17px] font-medium text-gray-400">Standardwagen:</label>
        ...
        <div className="flex items-center justify-between">
          <label className="text-[17px] font-medium text-gray-400">Steuerwagen:</label>
          <div className="flex items-center gap-2 bg-[#0A111C] border border-[#223041] rounded-lg px-2 py-1">
            <button
              type="button"
        ...
        <div className="flex gap-4 mt-6">
          <button
            type="button"
            className="flex-1 rounded-xl bg-[#783A32] py-3 font-semibold hover:bg-red-800"
            onClick={() => onClose?.()}
          >
            Abbrechen
          </button>
          <button
            type="submit"
            className="flex-1 rounded-xl bg-[#3D7041] py-3 font-semibold hover:bg-green-700"
          >
            Bestätigen
          </button>
        </div>
